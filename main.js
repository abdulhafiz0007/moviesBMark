    ...
    if(evt.target.matches(".js-bookMark-button")) {
        ...
        movies.find(item => {
            if(item.imdb_id === btnImdbId) {
                ...
                const isMovieInCanvasList = [...elCanvasList.querySelectorAll(".bookmark-list-item")].some(movie => {
                    return movie.dataset.imdbId === btnImdbId;
                });

                if(!isMovieInCanvasList) {
                    elCanvasList.appendChild(templateCloneNodes)
                } else {
                    alert("This movie already in Playlist")
                }
